...
import { Database, Loader2, Search, AlertTriangle, CheckCircle2, RefreshCw } from "lucide-react";
        ...
        <Card className="mb-4">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <input
              ...
              placeholder="CUSA00001_00 or UP9000-CUSA00001_00-LABEL"
              maxLength={36}
              onKeyDown={(e) => {
            ...
            <input
              ...
              placeholder="Region"
              maxLength={8}
              title="Region prefix (e.g. UP9000 for US, EP1018 for EU). Leave empty to search all regions."
              onKeyDown={(e) => {
                if (e.key === "Enter") void handleFetch();
              }}
            />
            ...
            <Button variant="primary" onClick={() => void handleFetch()} disabled={loading || !titleId.trim()}>
              {loading ? <Loader2 size={16} className="animate-spin" /> : <Search size={16} />}
              {tr("tmdb_fetch", undefined, "Fetch")}
            </Button>
          </div>
          <div className="mt-2 text-xs text-[var(--color-muted)]">
            Region prefix (e.g. UP9000=US, EP1018=EU, JP9000=JP) speeds up lookup by trying only that region instead of all 24.
          </div>
        </Card>
              ...
              <div className="grid grid-cols-1 gap-3 text-sm md:grid-cols-2">
                {result.np_title_id && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[var(--color-muted)]">NP Title ID:</span>
                    <code className="font-mono">{result.np_title_id}</code>
                  </div>
                )}
                {result.content_id && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[var(--color-muted)]">Content ID:</span>
                ...
                {result.publisher && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[var(--color-muted)]">Publisher:</span>
                    <span>{result.publisher}</span>
                  </div>
                )}
                {result.release_date && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[var(--color-muted)]">Release Date:</span>
                    <span>{result.release_date}</span>
                  </div>
                )}
                {result.genre && (
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-[var(--color-muted)]">Genre:</span>
                ...
                {result.icon && (
                  <div className="md:col-span-2">
                    <img src={result.icon} alt="icon" className="h-32 w-32 rounded-lg" />
                  </div>
                )}
                {result.description && (
